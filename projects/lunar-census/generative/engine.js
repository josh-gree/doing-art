// "Lunar Census, Plate IV" as a CFG-driven engine.
//
// Derived from projects/lunar-census/artgarten/sketch.js, which stays in the repo untouched
// as the reference render and the provenance. Everything from the seeded RNG
// down is copied VERBATIM — rnd, vnoise, warpPt, shadeAt, drawCensus,
// satelliteCensus, the dry-ring press, the paper loop, the finish pass. They
// carry the visual identity and they were not improved. What changed is that
// the ladder, the lattice, the gate, the seam, the ground and the ornament now
// read from CFG instead of from literals.
//
...
var PAPER = [229, 226, 217],
  CHARC = [45, 41, 37];
var GROUND, LINE_INK, FRAME_INK, FRAME_INK2, SHADOW_INK, SPECK_INK;
var LX = -0.45,
  LY = -0.60,
  LZ = 0.66;
var lightAz = 0,
...
function currentSeed() {
  var q = new URLSearchParams(window.location.search).get("seed");
  if (q) return q;
...
function setup() {
  ...
  pixelDensity(1);
  var c = createCanvas(CFG.canvas, CFG.canvas);
  c.parent("sketch");
  noLoop();
...
// The gate is the loudest parameter in the piece: it is the only thing that
// gives the outer census cloud a silhouette. Binary predicates are fine — this
// answers "cull this tick?", not "how far outside am I?".
function makeGate(g) {
  var norm = function (th) {
    return ((th % TWO_PI) + TWO_PI) % TWO_PI;
  };
  if (g.mode === "none") return function () {
    return false;
  };
  if (g.mode === "wedges") return function (th) {
    return Math.floor(norm(th) / (TWO_PI / g.wedges)) % 2 === 0;
  };
  if (g.mode === "half") return function (th) {
    return Math.cos(th - g.halfAngle) < 0;
  };
  if (g.mode === "annulus") return function (th, r) {
    return r > g.band[0] * RMAX && r < g.band[1] * RMAX;
  };
  if (g.mode === "checker") return function (th, r) {
    var w = Math.floor(norm(th) / (TWO_PI / g.wedges));
    var b = Math.floor(r / (RMAX / g.checkerRing));
    return (w + b) % 2 === 0;
  };
  // "noise": the delivered gate, with its threshold scaled.
  return function (th, r, ring, cs, sn) {
    var wth = (0.06 + 0.11 * vnoise(cs * 0.55 + 2, sn * 0.55 + 2) + ring * 0.015) * g.erode;
    if (wth > 0.22 * g.erode) wth = 0.22 * g.erode;
    return vnoise(cs * 1.2 + 21 + ring * 0.05, sn * 1.2 + 21) < wth;
  };
}
...
function draw() {
  ...
  rndS = CFG.rndSeed;
  noiseK = CFG.noiseK;
  randomSeed(CFG.rndSeed);
  noiseSeed(CFG.noiseK);

  CX = CFG.cx;
  CYm = CFG.cy;
  RMAX = CFG.rmax;
  RMIN = CFG.rmin;
  DIN = Math.round(CFG.moonR * 1.1);
  L = CFG.lattice;
  LADDER = CFG.ladder;

  MID_S = CFG.seam.mid;
  TILT = CFG.seam.tilt;
  T_PINCH = Math.max(1, CFG.seam.pinch);
  PIN_PULL = CFG.seam.pinch > 0 ? CFG.seam.pull : 0;

  // Ground and every grey that has to move with it. Inverting the sheet alone
  // leaves the halftone shadow and the plate frame painting dark on dark —
  // that was the one honest flaw in the study's ground-dark cell.
  GROUND = CFG.ground;
  PAPER = GROUND.paper;
  CHARC = GROUND.charc;
  LINE_INK = GROUND.line;
  FRAME_INK = GROUND.frame[0];
  FRAME_INK2 = GROUND.frame[1];
  SHADOW_INK = GROUND.shadow;
  SPECK_INK = GROUND.speck;
  ...
  SPH = [M];
  for (var s = 0; s < CFG.sats.length; s++) SPH.push(CFG.sats[s]);
  TICKS = [];
  ORBITS = [];
  GATE = makeGate(CFG.gate);
  for (s = 1; s < SPH.length; s++) buildOrbit(M, SPH[s], CFG.sats[s - 1].bow);
  buildCensus();

  drawPaper(ctx);
  tonalWashes();
  showThrough(M, 1.0);
  for (s = 1; s < SPH.length; s++) showThrough(SPH[s], 0.6 - 0.1 * (s - 1));

  guideCircles(ctx);
  if (CFG.ornament.halftone) halftoneShadow(M);
  dust();
  drawCensus();
  for (s = 1; s < SPH.length; s++) satelliteCensus(SPH[s], L.satGap, L.satStep);

  if (CFG.seam.stitches) {
    seatSmudge();
    stitches();
  }
  if (CFG.ornament.zigzag) {
    zigzag(CFG.ornament.zigzagY[0], -1);
    zigzag(CFG.ornament.zigzagY[1], 1);
  }
  dryRings();

  if (CFG.ornament.construction) constructionAll(ctx);
  if (CFG.ornament.orbits) orbitLines(ctx);
  if (CFG.ornament.strays) strays(ctx, M);
  for (s = 0; s < SPH.length; s++) coreFlecks(ctx, SPH[s]);
  if (CFG.ornament.centralMark) centralMark();
  if (CFG.ornament.legend) legend();
  registration(ctx);

  spatterAll();
  if (CFG.ornament.seal) seal();
  plateFrame(ctx);
  vignette(ctx);
  speckGrain();
  grainPass();
...
function keyPressed() {
  if (key === "r") {
    ...
    saveCanvas("lunar-census-" + CFG.seed, "png");
  }
}
...
function buildCensus() {
  ...
  while (r >= RMIN) {
    var inS = r < M.r;
    var gap = inS ? L.gapIn : L.gapOut;
    var step = inS ? L.stepIn : (L.stepOut + L.stepVar * vnoise(ring * 0.16, 3.7));
    var n = Math.max(8, Math.floor(TWO_PI * r / step));
    var phase = ring * L.phase;
    for (var k = 0; k < n; k++) {
      var th = phase + (k / n) * TWO_PI + rndr(-0.5, 0.5) * (TWO_PI / n) * L.aJit;
      var cs = Math.cos(th),
        sn = Math.sin(th);
      if (!inS && r > M.r * 1.06 && GATE(th, r, ring, cs, sn)) continue;
      var rr = r + (vnoise(cs * 1.4 + 5 + ring * 0.03, sn * 1.4 + 5) - 0.5) * 32 * L.rJit + rndr(-6, 6) * L.rJit;
      var x = CX + rr * cs,
        y = CYm + rr * sn;
      var wp = warpPt(x, y, Math.max(rr, 1));
      var tilt = (vnoise(x * 0.0013 + 9, y * 0.0013 + 9) - 0.5) * 2.0 +
        rndr(-0.06, 0.06) + 0.1 * Math.sin(th * 4 + r * 0.01);
      var base = th + CFG.orient + tilt;
      var bx = (1 - wp.pin) * Math.cos(base) + wp.pin;
      var by = (1 - wp.pin) * Math.sin(base);
      var ln = rndr(L.len0, L.len1),
        wg = rndr(L.wgt0, L.wgt1);
      if (inS) {
        ln *= 0.9;
        wg = rndr(L.wgtIn0, L.wgtIn1);
      }
      ln *= 1 + 0.55 * wp.pin;
      wg *= 1 + 0.3 * wp.pin;
      TICKS.push({
  ...
  while (r >= 8) {
    ...
    for (var k = 0; k < n; k++) {
      ...
      list.push({
        ...
        y: wp.y,
        ang: Math.atan2(by, bx),
        len: rndr(7, 13) * (S.r > 140 ? 1 : 0.85) * L.satK,
        wgt: rndr(2, 3.6) * L.satK,
        rr: rr,
        pin: wp.pin
      });
    }
...
function halftoneShadow(S) {
  var cw = CFG.ornament.halftoneCw,
    ang = -0.16;
  var ca = Math.cos(ang),
    sa = Math.sin(ang);
  ...
  for (var l = -Rn; l <= Rn; l++) {
    for (var k = -Rn; k <= Rn; k++) {
      ...
      if (r < 1.8) continue;
      fill(SHADOW_INK[0], SHADOW_INK[1], SHADOW_INK[2], 48 + 58 * tone);
...
function dust() {
  noStroke();
  for (var i = 0; i < CFG.press.dust; i++) {
    var a = rnd() * TWO_PI;
    var rd = Math.sqrt(DIN * DIN + (RMAX * RMAX - DIN * DIN) * rnd());
    var x = CX + rd * Math.cos(a),
      y = CYm + rd * Math.sin(a);
    if (x < FR.x0 + 6 || x > FR.x1 - 6 || y < FR.y0 + 6 || y > FR.y1 - 6) continue;
    if (rd > (DIN + RMAX) / 2) fill(96, 106, 122, rndr(38, 120));
...
function zigzag(yc, dir) {
  ...
  for (var p = 0; p < 2; p++) {
    ...
    for (i = 1; i < pts.length; i++) {
      ...
      if (al < 4) continue;
      ctx.strokeStyle = rgba3(LINE_INK[0], LINE_INK[1], LINE_INK[2], al / 255);
...
function dryRings() {
  var R = CFG.ornament.rings;
  for (var i = 0; i < R.length; i++) ringDry(R[i].r, R[i]);
}
...
function stitches() {
  stroke(LINE_INK[0], LINE_INK[1], LINE_INK[2], 215);
  strokeWeight(7.5);
  strokeCap(ROUND);
  ...
  while (x < FR.x1 - 50) {
    ...
    x += rndr(CFG.seam.stitchGap[0], CFG.seam.stitchGap[1]);
  }
}
...
function constructionAll(ctx) {
  ...
  if (CFG.ornament.ghostDial) ghostDial(ctx, 2712, 1745, 88);
}
...
function centralMark() {
  ...
  for (var j = 0; j < 5; j++) {
    var HC = LADDER[LADDER.length - 1].c;
    fill(HC[0], HC[1], HC[2], rndr(34, 64));
...
function drawPaper(ctx) {
  ...
  for (var y = 0; y < gh; y++) {
    for (var x = 0; x < gw; x++) {
      ...
      var v = GROUND.base + n1 * 26 + n2 * 11 + Math.sin(y * 0.055 + Math.sin(x * 0.01) * 2) * 2;
      var dx = x / gw - 0.5,
        dy = y / gh - 0.5;
      var dd = Math.sqrt(dx * dx + dy * dy) / 0.707;
      v *= 1 - 0.05 * Math.pow(dd, 2.6);
      v = Math.min(GROUND.clamp[1], Math.max(GROUND.clamp[0], v));
      var i4 = 4 * (y * gw + x);
      g.pixels[i4] = v + GROUND.tint[0];
      g.pixels[i4 + 1] = v + GROUND.tint[1];
      g.pixels[i4 + 2] = v + GROUND.tint[2];
      g.pixels[i4 + 3] = 255;
    }
  }
  g.updatePixels();
...
function tonalWashes() {
  noStroke();
  fill(251, 248, 239, CFG.press.wash[0]);
  ellipse(W * 0.30, H * 0.20, 2200, 1500);
  fill(120, 110, 95, CFG.press.wash[1]);
...
function spatterAll() {
  ...
  var seamY = seamAt(W / 2),
    i, x, y;
  for (i = 0; i < CFG.press.spatterSeam; i++) {
    x = rndr(FR.x0, FR.x1);
    y = seamY + (rnd() + rnd() + rnd() - 1.5) * 380;
    y = Math.min(FR.y1, Math.max(FR.y0, y));
    fill(SPECK_INK[0], SPECK_INK[1], SPECK_INK[2], rndr(35, 130));
    circle(x, y, rndr(1, 4.6));
  }
  for (i = 0; i < 26; i++) {
    x = rndr(FR.x0 + 60, FR.x1 - 60);
    y = seamAt(x) + rndr(-70, 70);
    fill(SPECK_INK[0], SPECK_INK[1], SPECK_INK[2], rndr(30, 90));
    circle(x, y, rndr(0.8, 2.4));
  }
  for (i = 0; i < CFG.press.spatterAll; i++) {
    x = rndr(FR.x0, FR.x1);
    y = rndr(FR.y0, FR.y1);
    var big = rnd() < 0.10;
  ...
  for (i = 0; i < 14; i++) {
    ...
    fill(SPECK_INK[0], SPECK_INK[1], SPECK_INK[2], rndr(60, 150));
...
function plateFrame(ctx) {
  wobbleRect(ctx, 92, 4.5, rgba3(FRAME_INK[0], FRAME_INK[1], FRAME_INK[2], FRAME_INK[3]));
  wobbleRect(ctx, 108, 1.4, rgba3(FRAME_INK2[0], FRAME_INK2[1], FRAME_INK2[2], FRAME_INK2[3]));
  ctx.strokeStyle = rgba3(FRAME_INK[0], FRAME_INK[1], FRAME_INK[2], 0.6);
  ctx.lineWidth = 2.4;
  ctx.lineCap = 'round';
  var mx = 1500,
    my = 1500;
...
function vignette(ctx) {
  ...
  g.addColorStop(1, 'rgba(88,82,70,' + CFG.press.vignette.toFixed(3) + ')');
  ctx.fillStyle = g;
...
function speckGrain() {
  noStroke();
  for (var k = 0; k < CFG.press.speck; k++) {
    var dk = rnd() < 0.55;
